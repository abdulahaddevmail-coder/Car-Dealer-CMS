...
export async function seedClassifieds(prisma: PrismaClient) {
  ...
  for (let i = 0; i < 25; i++) {
    ...
    classifiedsData.push({
      ...
      makeId: make.id,
      modelId: model.id,
      ...(variant?.id && { modelVariatId: variant.id }),
      title,
      ...
      currency: CurrencyCode.PKR, //faker.helpers.arrayElement(Object.values(CurrencyCode)),
      odoUnit: faker.helpers.arrayElement(Object.values(OdoUnit)),
      bodyType: faker.helpers.arrayElement(Object.values(BodyType)),
      transmision: faker.helpers.arrayElement(Object.values(Transmision)),

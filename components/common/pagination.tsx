...
export const Pagination = (props: PaginationProps) => {
  const { totalPages, maxVisiblePages = 5, styles, baseURL = "/inventory" } = props;
  ...
  return (
    <PaginationRoot className={styles.pRoot}>
      <PaginationContent className="lg:gap-4 justify-end">
        <PaginationItem>
          <PaginationPrevious
            className={cn(currentPage <= 1 && "hidden", styles.pPrev)}
          ...
          return (
            <PaginationItem key={pageNumber}>
              <PaginationLink
                ...
                className={cn(styles.pLink, isActive && styles.pLinkActive)}
                {...(rel ? { rel } : {})}
              >
                {pageNumber}
              </PaginationLink>
        ...
        <PaginationItem>
          <PaginationNext
            className={cn(currentPage >= totalPages && "hidden", styles.pNext)}
            href={createPageUrl(currentPage + 1)}
            onClick={(e) => {
              e.preventDefault();
